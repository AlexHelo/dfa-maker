...
if option == '1':
    ...
    # We create a isValid String to check if its valid inside the loops and conditionals
    isValid = 0


# Ask for string and make it a list (an array of chars for the string) and name it toValidate
    toValidate = input("Enter String to Validate: ")
    toValidate = list(toValidate)

# In this first for whe check that all letters in the sring are in our DFA's alphabet/ language
# we do this by checking if letter in the string to validate with each letter in our alphabet/language.
# We modify the isValid string by one each time it belongs and we check at the end that that number is the same as the current length we are checking
# it's not the fanciest solution but it works
...
# We reset the isValid string since we use it again to check the given string
    isValid = 0


# In this set of loops we check that the nodes are actually able to mak the string.
# We grab our currentState (which always starts in the initial state) with every single
# node in our test file while also checking the index of the string to validate.
#
# E.g. with a string that starts with a it will make the combination q0,a at the start and check
# with all possible nodes in our dictionary which have the same syntax.
#
# If this works it will keep moving through the letters of the string toValidate, if it ever breaks it will say so
    for x in range(len(toValidate)):
...
# Finally it checks that the last state we are in is actually a final state
